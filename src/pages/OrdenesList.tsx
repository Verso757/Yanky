          ...
          <TableBody>
            {filteredOrdenes.map((ot) => (
              <TableRow key={ot.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <TableCell>
                  <Badge variant="secondary" className={`${getStatusColor(ot.estado)} border-transparent font-medium`}>
                    {ot.estado?.replace("_", " ")}
                  </Badge>
        ...
        {filteredOrdenes.map((ot) => (
          <div key={ot.id} className="bg-white rounded-lg border border-slate-200 p-4 shadow-sm space-y-4">
            <div className="flex justify-between items-start">
              ...
              <Badge variant="secondary" className={`${getStatusColor(ot.estado)} border-transparent`}>
                {ot.estado.replace("_", " ")}
              </Badge>
            </div>
